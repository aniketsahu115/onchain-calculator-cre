...
var onCronTrigger = (runtime2) => {
  ...
  const onchainValue = decodeFunctionResult({
    abi: Storage,
    functionName: "get",
    data: bytesToHex3(contractCall.data)
  });
  ...
  const txHash = updateCalculatorResult(runtime2, network286.chainSelector.selector, evmConfig, offchainValue, onchainValue, finalResultValue);
...
function updateCalculatorResult(runtime2, chainSelector, evmConfig, offchainValue, onchainValue, finalResult) {
  ...
  const txHash = bytesToHex3(writeReportResult.txHash || new Uint8Array(32));
